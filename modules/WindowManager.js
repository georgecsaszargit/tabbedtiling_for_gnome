    ...
    connectSignals() {
		...
		global.get_window_actors().forEach(actor => {
		    ...
		    if (win && win.get_window_type() === Meta.WindowType.NORMAL) {
		        this._connectWindowStateSignals(win);
		    }
		});
...
_getZoneTabBar(zoneId, monitorIndex, zoneDef) {
    ...
    _unsnapWindow(window, keepCurrentPosition = false) {
		...
		log('_unsnapWindow', `Unsnapping "<span class="math-inline">\{window\.get\_title\(\)\}" from zone "</span>{oldZoneId || 'N/A'}". keepCurrentPosition=${keepCurrentPosition}`); // 
    ...
    _connectWindowStateSignals(window) {
		...
		const signalIds = [
		    window.connect('notify::fullscreen', handler), // 
		    window.connect('notify::maximized-horizontally', handler), // 
		    window.connect('notify::maximized-vertically', handler), // 
